...
- `npm run mlp:export-ai` exports a non-image AI handoff document. It writes `exports/ai-handoff.md` for local sharing and `public/ai-handoff.md` so static deployments can expose `/ai-handoff.md`. The export must include project metadata, page directory, page states and routes, text-based prototype layout trees, interaction mappings, Product Notes, Test Cases, and `checked` status. This is the default way to give other AI systems prototype context without relying on screenshots or JS execution.
- Product Notes may support local in-browser editing during `vite dev` only. Each interaction card can show an `编辑` button in local dev, switch into a structured editor, and save through the local framework API `POST /__mlp/api/notes/update`. Static builds and online deployments remain read-only until a real authenticated backend exists. Local edits are stored in `src/project/notes/local-edits.json` and merged over `src/project/notes/interactions.js` at runtime; do not write arbitrary browser input directly into JS source files.
- Local Product Notes editing must handle the first-stage safeguards: field validation, save failure retaining the user's draft, comparable-field hash conflict checks for concurrent edits, automatic `checked: true`, `checkedBy: 'human'`, and `reviewed: false` after any human saved change, and unsaved-change interception before page/state/hash switching or browser unload. `checked` means the item has been manually checked or manually edited; it is not a lock. Humans can continue editing checked items, and every 页面说明、交互说明 card, and individual 测试用例 card must allow toggling between `checked` and `未 check` without entering the text editor. AI documentation generation, rebuild, migration, and framework sync must read checked items but must not overwrite checked text by default; they may only add missing new entries or fill empty fields unless the user explicitly asks to rewrite checked documentation. If the local save API is missing, the UI should fail clearly and keep the draft in the editor.
...
- **Project/settings rail:** Put project card, theme card, interaction guide card, and related content card in an independent first column named `project-settings-rail`. It should contain project name, short positioning/path, status summary when useful, the only light/dark phone theme switch, the `交互引导` switch, and a single `相关内容` card. Theme and Guide controls use the same two-option segmented interaction style: Theme is `亮色 / 暗色`, Guide is `关闭 / 开启`. Do not place project/theme/guide controls inside the page directory column or Product Notes panel. The right documentation panel mode is controlled inside the right panel itself, not from the settings rail.
- **Interaction guide shortcut:** Pressing `G` toggles `交互引导` in the review workspace. Do not trigger the shortcut while the user is typing in an input, textarea, select, contenteditable area, or while modifier keys are held. The Guide card must show the `G` shortcut hint.
- **Related content card:** The project/settings rail must include one `相关内容` card, using the same `theme-card` shell and `settings-doc-list` list style. Put `指令查看` and `获取 .md` as the first two entries in this card, followed by support pages such as component library, test acceptance checklist, and changelog. Do not create separate `COMMANDS`, `AI HANDOFF`, or `DOCS/交付文档` groups. `指令查看` opens a workbench modal listing common MLP operations as simple Chinese natural-language prompts and their mapped commands. `获取 .md` opens the AI handoff modal that can copy generated Markdown content, copy the deployed `ai-handoff.md` link, and copy the local `exports/ai-handoff.md` path. If `ai-handoff.md` has not been generated, the modal must tell the user to run `npm run mlp:export-ai`. These entries are framework chrome, not phone prototype elements: do not list them in Product Notes, Test Cases, UI checklist, or page interaction connectors. The command modal must include at least daily editing, route/state checks, documentation output, non-visual acceptance, explicit visual acceptance, project initialization, framework sync, rebuild from legacy, release/deploy, and version rollback prompts. Visual-review commands must be labeled explicit-only.
...
- **Center prototype:** Show a persistent prototype-only state switch above the phone for every page. Remove the leading "状态" label; show only compact state chips sized to their text. The state switch wrapper and unselected chips must be transparent, with no gray/framed background; only the active chip uses a filled selected background. If all chips fit within the 375px switch, the whole chip group must be horizontally centered. If chips overflow, the horizontal scroller must include explicit left/right edge centering space, such as `--state-switch-edge-space`, `scroll-padding-inline`, and matching overflow-only inner `padding-inline`, so the first and last active capsule can center without being clipped. If the page has one state, still show one `默认态` chip and center it. Do not add custom mouse-drag behavior to the state switch by default. Clicking a state chip must switch states, detect overflow, and when overflow exists scroll the active chip into the horizontal center of the state switch.
...
- **Documentation panel structure:** For every confirmed page state, the right panel has a sticky top bar containing page title, page number, state number, and a three-option tab control: `页面说明 / 交互说明 / 测试用例`. Do not control this panel from the left settings rail. `页面说明` summarizes page function, parent page, child pages, and the page element list. `交互说明` documents only real phone UI elements in the current page state. `测试用例` is a separate right-side view and must not be embedded inside interaction cards. Test cases must be organized by the same current-state interaction elements and component numbers as Product Notes.
- **Interaction documentation fields:** Use professional component-delivery structure, not conversational labels. Every interaction card in `交互说明` must render exactly these sections: `1. 组件基本信息` only shows component type; `2. 组件构成` describes what visible/content sub-elements the component contains; `3. 状态定义` shows only the state names that actually exist for this mobile app element, and local editing uses multi-select state tags rather than long text fields; `4. 交互行为` is one narrative text block using circled sequence numbers such as `①②③` to describe each operation and its result, without separate trigger/feedback labels; `5. 数据说明` explains required data fields, their types, and whether each value is frontend static/configured or backend/server/user-provided. Do not mechanically generate desktop `悬停态` or generic `按下态`; mobile prototypes usually should not include them unless the user explicitly asks for touch feedback documentation. State definitions must be derived from the element function: normal buttons usually use `默认态` and `禁用态` only, async buttons may add `加载态`; input fields may add `聚焦态` and `错误态`; upload controls may add `上传中`, `上传成功`, `上传失败`; selectable materials/templates/categories may add `选中态` and `未选中态`; switches/checkboxes use `开启态/关闭态` or `勾选态/未勾选态`; lists/cards fed by backend data may add `加载态`, `空数据态`, and `加载失败态`; modal/sheet/toast/drawer elements may add `展开态/收起态` or `显示态/隐藏态`; media preview may add `播放态/暂停态/加载失败态`. Temporarily do not show boundary rules, animation details, or design parameters inside interaction cards. Show required rules, validation boundaries, and upload/select limits only inside data说明 when the element actually needs input, selection, or upload. Show empty/failure data behavior only for backend/API/list/media data elements. Recommended element types include `按钮`, `输入框`, `文本`, `图片上传`, `文件上传`, `视频上传`, `开关`, `单选`, `多选`, `下拉选择`, `标签切换`, `图片/封面`, `列表/卡片`, `弹窗`, `Sheet`, and `Toast`.
- **Documentation local editing:** In local dev only, 页面说明、交互说明、测试用例 cards may expose `编辑 / 完成 / 取消`. 交互说明按元素卡片单独编辑；测试用例必须按每一条用例卡片单独编辑，不能用一个交互元素级别的补充说明替代多条用例编辑。Editing must use structured fields, validate required content, preserve unsaved text after save failure, write edits to `src/project/notes/local-edits.json` through the local Vite API, use comparable-field hashes to detect concurrent changes, and set changed items to `checked: true`, `checkedBy: 'human'`, and `reviewed: false`. Each card also has a direct check toggle: `checked` can be changed back to `未 check` without opening the editor, and test cases toggle at individual case-card granularity. `checked` is a manual-confirmation marker, not a lock: human editors may keep editing it, while AI scripts must avoid overwriting checked text unless explicitly instructed. The workspace must block page, state, hash, and browser unload transitions while edits are dirty unless the user confirms discarding the draft. Static cloud deployments are read-only.
...
Prototype UI and interaction documentation must follow `复用性 + 独立性` as the decomposition standard:

- **复用优先:** If the same UI block appears in two or more pages or states, extract it into a reusable component or shared page fragment. Examples: template cards, work cards, login form fields, bottom action bars, upload areas, and plan cards.
- **功能内聚:** A component should own one clear responsibility. An input owns input display, focus, value, validation, and error state; submit behavior belongs to a button or parent flow. A card owns its display and click surface; navigation/result mutation stays in the parent callback.
- **可独立配置:** Avoid hard-coded component internals. Pass copy, size, selected/disabled/loading/error state, data fields, callbacks, and slots as props/config. Do not create duplicate components such as separate phone-code and email-code fields when one configurable `FormField` pattern can cover both.
- **三层拆分:** Use `原子组件` for indivisible units such as button, input, icon, avatar, tag, and placeholder image; `分子组件` for composed functional blocks such as search bar, product/template card, upload field, and verification-code row; `区块组件` for page regions such as top nav, banner area, waterfall list, bottom tab, toolbar, drawer, sheet, and modal content.
- **该拆:** repeated UI, complex internal interaction, independent lifecycle such as loading/empty/error, or parts that can unblock parallel work should be extracted.
- **不该拆:** one-off static copy, page-only layout with no reuse expectation, or components that require excessive prop plumbing and become harder to understand after extraction should stay inline.
- **实操流程:** First complete the page sketch, then scan all pages/states for repeated visual or behavioral blocks, draw candidate component boundaries, define each candidate's input data, output events, and internal state, then validate by reusing it in another page/state without internal code changes.
- **Documentation mapping:** Product Notes should document the reusable component as a component card when it has product meaning, and also document each interactive child control or meaningful content child when it has its own behavior, data source, backend field, state, or acceptance requirement. A `TemplateCard` can have a card-level click note, while its cover, title, tag, price, and video icon may be separate content elements when their data source or failure state matters.
...
6. Add missing framework logic such as the centered update toast, `version.json`, and `scripts/write-version.mjs` when the project lacks them.
7. Run the skill's framework guard sync script before final verification:
   ...
   The guard sync must also preserve protected workbench visual details: Theme and Guide cards use the same hard-coded compact card/toggle style; the right documentation panel uses its own sticky top bar and three-option tab control; prototype state chips stay transparent except for the active chip; Product Notes and Test Cases stay as separate right-side views; and primary tabs must not add an extra black bottom shell block.
...
- Interaction element granularity should mirror component decomposition. Document reusable component blocks at the molecule/block level when the block has product behavior, but also split meaningful atomic children when they have independent data source, state, validation, backend field, or user action. For example, a template card may have a card click interaction, a cover content element with image loading/failure rules, a title content element with truncation/data source, and a video icon content element when the media type changes behavior.
...
- `交互说明`: only for real phone UI controls and visible content elements. Each interaction card must use the current five-section component-delivery structure: `组件基本信息`（只保留组件类型）, `组件构成`, `状态定义`（只展示真实存在的状态标签，编辑时多选）, `交互行为`（用 `①②③` 分段的一整段文本描述操作和结果）, and `数据说明`（字段、类型、前端写死/配置或后端传入/用户产生）。Do not show boundary rules, animation, or design parameter sections in the interaction card. Do not add `悬停态` or `按下态` by default; most mobile app prototype elements do not need them. Show required rules, validation boundaries, and input/upload/select limits only when they are part of the data说明 for an actual input, selection, or upload element. Show empty/failure data behavior only for backend/API/list/media data elements. Do not show irrelevant template fields.
...
- Treat requests such as "部署", "部署一下", "部署线上", "发到服务器", "更新线上", "上线", "发布到云端", or "给我线上地址" as explicit cloud deployment intent. Treat "同步到阿里云" as explicit Aliyun intent only when the user says Aliyun in the current task; otherwise do not use the historical Aliyun server.
...
- For the configured prototype server, read `references/deployment.md` and resolve deployment settings from user-provided values and `MLP_DEPLOY_*` environment variables. The default subdomain is `prototype.flnisbest.xyz`, but the remote host/user/root must point to the current non-Aliyun hosting target. If Spaceship SSH/SFTP host, user, root, or authentication is missing, ask for those details instead of falling back to Aliyun.
- Cloud deployment is project-level. Deploy `<MLP_PROJECT_ROOT>/<project-slug>/` to `https://${MLP_DEPLOY_DOMAIN:-prototype.flnisbest.xyz}/<project-slug>/`; do not deploy misplaced workspace-local project folders or page/state URLs.
- Every project build should update `public/version.json` before Vite builds. The front end should read `version.json` on load, poll it every 180 seconds with a cache-busting query string, and show the fixed screen-centered update toast when the remote version changes. Never auto-refresh the page.
